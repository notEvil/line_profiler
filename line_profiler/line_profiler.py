...
class LineProfiler(CLineProfiler):
    ...
    def print_stats(self, stream=None, output_unit=None, stripzeros=False):
        ...
        """
        lstats = self.get_stats()
        show_text(lstats.timings, lstats.unit, output_unit=output_unit, stream=stream, stripzeros=stripzeros, block_profiles=getattr(lstats, "block_profiles", None))
...
def show_func(filename, start_lineno, func_name, timings, unit,
              output_unit=None, stream=None, stripzeros=False, block_profile=None):
...
def show_text(stats, unit, output_unit=None, stream=None, stripzeros=False, block_profiles=None):
    ...
    for key, timings in sorted(stats.items()):
        fn, lineno, name = key
        show_func(fn, lineno, name, timings, unit,
                  output_unit=output_unit, stream=stream,
                  stripzeros=stripzeros, block_profile=None if block_profiles is None else block_profiles.get(key))
...
def main():
    ...
    show_text(lstats.timings, lstats.unit, output_unit=args.unit, stripzeros=args.skip_zero, block_profiles=getattr(lstats, "block_profiles", None))
